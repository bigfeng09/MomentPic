...
> **AI 生成/辅助开发声明与责任说明**
>
> 本项目主要由 AI 助手在用户指导下完成、生成、重构和部署验证。项目目标是参考 MomentPic 这类自托管相册工具的使用场景、数据组织方式和迁移需求，重新实现一套 Backend V2、Web UI 与 Android 客户端。代码、文档、迁移脚本、部署模板和客户端实现都可能存在缺陷、遗漏或不适合你的环境之处。使用者应在使用、部署、迁移或二次开发前自行审查代码、测试功能、备份数据并承担相应风险。
>
...
MomentPic V2 是一个自托管相册与图片库项目，包含 Node.js 后端、内置 Web 管理界面和 Android 客户端源码。它面向已有 MomentPic 使用者可能遇到的旧库迁移、路径映射、多端访问和自托管部署需求进行重新实现，目标是在尽量兼容旧图库/相册/资源/用户授权关系的基础上，提供更安全、更容易部署和维护的新版本。
...
Android 设置页主页面保留连接和外观选项，并提供两个并列入口：

- 账户管理：管理员可创建、改密、删除普通账号并管理相册分享；非管理员会看到权限提示。
- 数据管理：包含相册库目录登记、已登记图库来源列表、来源启用/禁用、`dryRun=true` 扫描预览、图片缓存清理和搜索历史清理。来源管理仅管理员可用，本机清理功能仍可使用。
...
本项目并非原 MomentPic 官方项目，也不代表原作者。项目开发过程中参考了 MomentPic 这类自托管相册工具的使用场景、数据组织方式和迁移需求，并围绕“旧库迁移、路径映射、Web 管理端、Android 客户端、Docker 部署”等目标重新实现 Backend V2、Web UI 与 Android 客户端。

原 MomentPic 仓库当前未声明明确开源许可证。公开仓库或公开镜像并不等同于授予复制、修改、再发布或商用授权。如果你需要使用、修改、分发原 MomentPic 源码或镜像，请自行确认其作者授权与许可证要求。

本仓库只声明当前仓库内脱敏发布源码、文档和部署模板的使用方式；不对原 MomentPic 的源码、镜像或授权状态作任何保证。
...
- 新增 Android 客户端源码和 debug APK 构建工作流。
- 新增 Docker Compose、GHCR 镜像和 GitHub Actions 构建流程。
