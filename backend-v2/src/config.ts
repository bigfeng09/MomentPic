...
const defaultPathPrefixMap: PathPrefixMapRule[] = [
  {
    from: "/mnt/user/media/download/moment/",
    to: "/mnt/user/media/download/moment/download/"
  }
];
...
export const loadConfig = (overrides: Partial<AppConfig> = {}): AppConfig => {
  ...
  return {
    ...
    authSecret: process.env.MOMENTPIC_AUTH_SECRET ?? process.env.MOMENTPIC_ADMIN_PASSWORD ?? "momentpic-v2-local-secret",
    adminUsername: (process.env.MOMENTPIC_ADMIN_USERNAME ?? "admin").trim().toLowerCase(),
    adminPassword: process.env.MOMENTPIC_ADMIN_PASSWORD ?? "admin123",
